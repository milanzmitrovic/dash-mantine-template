...
__init__.py 
    ...
    All functions that are defined inside segmented_control/ folder 
    and that should be used externally are imported in 
    __init__.py file so that they can be user as gc.segmented_control...
...
Component.py
    ...
    *** component()
    ...
    --> It will be imported and instantiated in all 
        places where SEGMENTED_CONTROL component is used 
        (There could be several components of type SEGMENTED_CONTROL, 
        and this is expected actually).

    --> We use template to create specific instance 
        of SEGMENTED_CONTROL component.
