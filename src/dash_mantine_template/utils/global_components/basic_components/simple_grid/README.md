...
__init__.py 
    ...
    All functions that are defined inside simple_grid/ folder 
    and that should be used externally are imported in 
    __init__.py file so that they can be user as gc.simple_grid...
...
Component.py
    ...
    *** component()
    ...
    --> It will be imported and instantiated in all 
        places where SIMPLE_GRID component is used (There 
        could be several components of type SIMPLE_GRID, and 
        this is expected actually).

    --> We use template to create specific instance 
        of SIMPLE_GRID component.

Input.py 
    
    This file does not exist. Why?
    
    simple_grid component is not meant 
