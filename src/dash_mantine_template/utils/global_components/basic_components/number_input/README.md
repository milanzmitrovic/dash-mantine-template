...
__init__.py 
    ...
    All functions that are defined inside number_input/ folder 
    and that should be used externally are imported in 
    __init__.py file so that they can be user as gc.number_input...
...
Component.py
    ...
    *** component()
    ...
    --> It will be imported and instantiated in all 
        places where NUMBER_INPUT component is used 
        (There could be several components of type NUMBER_INPUT, 
        and this is expected actually).
    
    --> We use template to create specific instance 
        of NUMBER_INPUT component.
