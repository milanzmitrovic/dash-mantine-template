"""
Purpose of this file is to hold function
that will help in organization of callback
OUTPUT related with number_input component.

Here we define what (default) property of
number_input component will be updated with
callback function.

<<<
number_input component is used as
OUTPUT of callback
<<<
<<<

Also, we wll be able to see what are all
places in which number_input was used
as OUTPUT in callback.
"""

from dash import Output
from pydantic import ConfigDict, validate_call


@validate_call(config=ConfigDict(arbitrary_types_allowed=True), validate_return=True)
def output(id_: str, property_: str = "value") -> Output:
    """
    Purpose of this function is to help with
    creation of callback signature.
    """

    return Output(component_id=id_, component_property=property_)


@validate_call(config=ConfigDict(arbitrary_types_allowed=True), validate_return=True)
def output__loading_overlay(id_: str, property_: str = "visible") -> Output:
    """
    Purpose of this function is to help with
    creation of callback signature.
    """

    return Output(component_id=id_ + "__loading-overlay", component_property=property_)
